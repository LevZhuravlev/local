...
class ViewWithRegistrationField: UIView {
    ...
    var isFieldOptional: Bool = false
    ...
    init(frame: CGRect,
         ...
         placeholder: String = "",
         isFieldOptional: Bool = false) {
        
        super.init(frame: frame)
        titleLabel?.text = name
        textFieldPlaceholder = placeholder
        self.isFieldOptional = isFieldOptional
    ...
    private func setDesriptionLabel() {
        
        if let desriptionLabel = desriptionLabel, textFieldPlaceholder != "" {
              ...
              let desriptionLabelHeigth = desriptionLabelSize!.height * (((desriptionLabelSize!.width + 64) / UIScreen.main.bounds.width)).rounded(.up)
                          
              desriptionLabel.translatesAutoresizingMaskIntoConstraints = false
              desriptionLabel.topAnchor.constraint(equalTo: titleLabel?.bottomAnchor ?? mainScrollView.topAnchor, constant: 16).isActive = true
    ...
    textFieldHeightAnchor = textField.heightAnchor.constraint(equalToConstant: 164)
    ...
    private func setContinueButton() {
        ...
        continueButton = {
            ...
            let attributeString = NSMutableAttributedString(string: "ПРОПУСТИТЬ",
                                                            attributes: yourAttributes)

            view.setAttributedTitle(attributeString, for: .normal)
            view.tintColor = .white
            
            view.backgroundColor = UIColor(red: 1, green: 0.96, blue: 0.858, alpha: 1)
            view.addTarget(nil, action: #selector(EventRegViewController.continueFiledTouched), for: .touchUpInside)
    ...
    @objc func keyboardWillShow(_ notification: Notification) {
        if let keyboardFrame: NSValue = notification.userInfo?[UIResponder.keyboardFrameEndUserInfoKey] as? NSValue {
            ...
            UIView.animate(withDuration: 0.5, delay: 0, usingSpringWithDamping: 1, initialSpringVelocity: 0.7, options: .curveEaseInOut) {
                ...
                self.continueButton?.transform = CGAffineTransform(translationX: 0, y: -keyboardHeight+8)
            }
        }
        if textField?.text.count ?? 0 > 64 {
    ...
    @objc func keyboardWillHide(_ notification: Notification) {
        ...
        if textField?.text.count ?? 0 > 64 {
            textFieldHeightAnchor?.isActive = false
            textFieldHeightAnchor = textField?.heightAnchor.constraint(equalToConstant: UIScreen.main.bounds.height - 260)
            textFieldHeightAnchor?.isActive = true
            textField?.layoutIfNeeded()
        }
    }
